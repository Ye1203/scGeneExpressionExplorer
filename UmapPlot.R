...
mp_plot_table <- function(df, title){
  table <- gridExtra::tableGrob(df, rows = NULL, theme = ttheme_minimal(base_size = 15))
  h <- grobHeight(table)
  w <- grobWidth(table)
...
UmapDotplotTableHeatmapPlot_v2 <- function(seurat_obj, 
  ...
  #### CREATE STATS TABLE WITH CLUSTERS
  clusters_stats_table_plot <- mp_plot_table(list_sample_cluster_stats(seurat_obj, cluster_ident)[[sample_id]], title = "")
...
#'
#' @examples
list_sample_cluster_stats <- function(seurat_obj, cluster_ident = "seurat_clusters"){
  ...
  all_cells <- seurat_obj@meta.data %>% 
    select(seurat_clusters = cluster_ident, sample_id, counts = nCount_RNA, genes = nFeature_RNA) %>% 
